...
def test(test_name, &block)
  return NewCliTest::Test.new(test_name, &block)
end
...
class Test
  ...
  # Provenance du test (structure comprenant :file et :line)
  attr_reader :from

  def initialize name, &block
    @description = name
    @from = extract_from(caller[2]) #=> {PlaceInFile}
  ...
  def inspect
    @inspect ||= begin
      "<<#{description} #{from.relpath}::#{from.line}>>"
    end
  end
  ...
  def exec_operation
    ENV['CLI_TEST_ANSWERS'] = @inputs.to_json
